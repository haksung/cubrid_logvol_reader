		  ...
		  bool is_long_num)
{
  ...
  /*  Shift *answer and *remainder.  Bits shifted out of *answer  *
   *  are placed into *remainder.                                 */
			 ...
			 int *dest_prec, int *dest_scale, DB_C_NUMERIC dest)
{
  ...
  /* If src precision fits without truncation, merely set dest to the lower
   * half of the source buffer and return */
  ...
  /*  The remaining cases are for when the precision of the source
   *  overflows.
   */
  ...
  /* Case 1:  The scale of the source does *not* overflow */
    ...
    {
      /* If upper half of *src is zero, merely copy, reset precision,
       * and return
       */
   ...
   * This means the number can't overflow as long as truncation
   * occurs. Reduce the scale and precision by the same amount.
   */
  else
    {
      ...
      *dest_prec = DB_MAX_NUMERIC_PRECISION;
      ...
      /* Truncate the obsolete trailing digits.
       * (Note: numeric_coerce_big_num_to_dec_str is guaranteed ro return
       *        a NULL-terminated buffer that is TWICE_NUM_MAX_PREC
       *        characters long.)
       */
		      ...
		      DB_VALUE * answer)
{
  ...
  /* In order to maintain the proper number of scaling in the output,
...
numeric_coerce_dec_str_to_num (const char *dec_str, DB_C_NUMERIC result)
{
  ...
  /* Check for a negative number. Negative sign must be in the first
   * decimal place
   */
  if (*dec_str == '-')
    {
			   ...
			   DB_C_NUMERIC dest_num)
{
  ...
  /* If src is negative, coerce the positive part now so that rounding
   * is always done in the correct 'direction'.
   */
  if (numeric_is_negative (src_num))
    {
      negate_answer = true;
