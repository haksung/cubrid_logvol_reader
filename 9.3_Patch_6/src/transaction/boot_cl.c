  ...
  if (ptr == NULL)
      ...
      if (db == NULL)
	{
	  ...
	  || (db->num_hosts > 1
		  ...
		  || client_credential->client_type == BOOT_CLIENT_LOG_COPIER
		  || client_credential->client_type == BOOT_CLIENT_LOG_APPLIER
		  || client_credential->client_type ==
		  BOOT_CLIENT_LOG_PREFETCHER)))
	{
  ...
  for (i = 0; i < 2; i++)
      ...
      else if (BOOT_CSQL_CLIENT_TYPE (client_credential->client_type))
	{
	  ...
	  break;		/* dont retry */
	       ...
	       || client_credential->client_type ==
	       BOOT_CLIENT_LOG_APPLIER_PLUGIN)
	{
	  if (i == 0)		/* first */
	    {
	      check_capabilities = true;
