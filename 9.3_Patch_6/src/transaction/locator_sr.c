		...
		int continue_on_error)
{
  ...
  for (i = 0; i < mobjs->num_objs; i++)
      ...
      if (i == 0)
	{
	  ...
	  || LOG_CHECK_LOG_APPLIER_PLUGIN (thread_p) || num_ignore_error > 0)
	{
				      ...
				      RECDES * recdes)
{
  ...
  if (btree_keyoid_checkscan_start (&tmp_btid, &bt_checkscan) != NO_ERROR)
    {
      error = ER_FAILED;
      goto free_and_return;
    }
...
free_and_return:
  ...
  if (bt_checkscan_p)
    {
      btree_keyoid_checkscan_end (bt_checkscan_p);
    }
