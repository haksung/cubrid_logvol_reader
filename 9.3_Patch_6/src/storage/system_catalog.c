...
catalog_get_btree_statistics (BTREE_STATS * stat_p, char *rec_p)
{
  ...
  if (stat_p->has_function > 0)
    {
      DB_MAKE_NULL (&(stat_p->unused_min_value));
      DB_MAKE_NULL (&(stat_p->unused_max_value));
    }
}
...
catalog_put_disk_attribute (char *rec_p, DISK_ATTR * attr_p)
{
  ...
  memcpy (rec_p + CATALOG_DISK_ATTR_MIN_VALUE_OFF, &attr_p->unused_min_value,
	  sizeof (DB_DATA));
  memcpy (rec_p + CATALOG_DISK_ATTR_MAX_VALUE_OFF, &attr_p->unused_max_value,
	  sizeof (DB_DATA));
...
catalog_put_btree_statistics (char *rec_p, BTREE_STATS * stat_p)
{
  ...
  if (stat_p->has_function > 0)
    {
      assert (DB_IS_NULL (&(stat_p->unused_min_value)));
      assert (DB_IS_NULL (&(stat_p->unused_max_value)));

      min_val_size = OR_VALUE_ALIGNED_SIZE (&stat_p->unused_min_value);
      max_val_size = OR_VALUE_ALIGNED_SIZE (&stat_p->unused_max_value);
    }
  ...
  if (stat_p->has_function > 0)
    {
      ...
      or_put_value (&buf, &stat_p->unused_min_value, 0, 1, 1);
      or_put_value (&buf, &stat_p->unused_max_value, 0, 1, 1);
    }
}
			      ...
			      DISK_ATTR * pre_attrs_p, int pre_attr_count)
{
  ...
  for (i = 0, new_attr_p = new_attrs_p; i < new_attr_count; i++, new_attr_p++)
	   ...
	   j++, pre_attr_p++)
	{
	  ...
	  new_attr_p->unused_max_value = pre_attr_p->unused_max_value;
	  new_attr_p->unused_min_value = pre_attr_p->unused_min_value;
...
catalog_btree_statistics_get_size (BTREE_STATS * bt_stats_p)
{
  if (bt_stats_p->has_function > 0)
    {
      assert (DB_IS_NULL (&(bt_stats_p->unused_min_value)));
      assert (DB_IS_NULL (&(bt_stats_p->unused_max_value)));

      return OR_VALUE_ALIGNED_SIZE (&bt_stats_p->unused_min_value) +
	OR_VALUE_ALIGNED_SIZE (&bt_stats_p->unused_max_value) +
