		   ...
		   int ispeeking, int chn)
{
...
#endif /* CUBRID_DEBUG */
  assert (ispeeking == COPY || (ispeeking == PEEK && scan_cache != NULL));
  ...
  use_scan_cache_area = false;
  if (scan_cache != NULL)
    {
      if ((ispeeking == COPY && recdes->data == NULL)
	  || (ispeeking == PEEK && scan_cache->cache_last_fix_page == false))
	{
	  use_scan_cache_area = true;
	  ispeeking = COPY;
	}
    }
