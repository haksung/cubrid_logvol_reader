...
applylogdb (UTIL_FUNCTION_ARG * arg)
{
  ...
  char er_msg_file[PATH_MAX];
  const char *database_name = NULL;
  const char *source_db_name = NULL;
  const char *log_path = NULL;
  char plugin_db_name[DB_MAX_IDENTIFIER_LENGTH];
  char log_path_buf[PATH_MAX];
  char *log_path_base;
  char *s;
  int max_mem_size = 0;
  ...
  plugin_mode = utility_get_option_bool_value (arg_map, APPLYLOG_PLUGIN_S);
  if (plugin_mode)
    {
      source_db_name =
	utility_get_option_string_value (arg_map, APPLYLOG_SOURCE_DB_S, 0);
    }
  ...
  if (plugin_mode)
    {
      db_set_client_type (DB_CLIENT_TYPE_LOG_APPLIER_PLUGIN);
    }
  else
    {
  ...
  if (plugin_mode || prm_get_bool_value (PRM_ID_HA_APPLY_PLUGIN))
    {
      max_mem_size =
	prm_get_integer_value (PRM_ID_HA_APPLY_PLUGIN_MAX_MEM_SIZE);
    }
  else
    {
      if (max_mem_size > 500)
  ...
  if (plugin_mode)
    {
      strncpy (plugin_db_name, database_name, DB_MAX_IDENTIFIER_LENGTH);
      s = strchr (plugin_db_name, '@');
      if (s)
	{
	  *s = '\0';
	}
      database_name = (const char *) plugin_db_name;
    }
#endif
...
retry:
  ...
  if (plugin_mode)
    {
      prm_set_bool_value (PRM_ID_HA_APPLY_PLUGIN, true);
    }

  if (plugin_mode)
    {
      error =
	la_apply_log_file (source_db_name, log_path, max_mem_size,
			   database_name);
    }
  else
    {
      error = la_apply_log_file (database_name, log_path, max_mem_size, NULL);
    }
  if (error != NO_ERROR)
    {
      fprintf (stderr, "%s\n", db_error_string (3));
...
/*
 * logreader() - LogReader main routine
 *   return: EXIT_SUCCESS/EXIT_FAILURE
 */
int
logreader (UTIL_FUNCTION_ARG * arg)
{
#if defined (WINDOWS)
  PRINT_AND_LOG_ERR_MSG (msgcat_message (MSGCAT_CATALOG_UTILS,
