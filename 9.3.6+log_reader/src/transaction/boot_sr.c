		       ...
		       BOOT_SERVER_CREDENTIAL * server_credential)
{
  ...
  if (tran_index != NULL_TRAN_INDEX)
      ...
      if (BOOT_NORMAL_CLIENT_TYPE (client_credential->client_type))
	{
	  ...
	  || client_credential->client_type == BOOT_CLIENT_LOG_APPLIER_PLUGIN)
	{
...
xboot_unregister_client (THREAD_ENTRY * thread_p, int tran_index)
{
  ...
  if (BO_IS_SERVER_RESTARTED () && tran_index != NULL_TRAN_INDEX)
      ...
      if (tdes == NULL || tdes->client_id != client_id)
	{
	  ...
	  || tdes->client.client_type == BOOT_CLIENT_LOG_APPLIER_PLUGIN)
	{
	  css_notify_ha_log_applier_state (thread_p,
					   HA_LOG_APPLIER_STATE_UNREGISTERED);
	}
...
xboot_checkdb_table (THREAD_ENTRY * thread_p, int check_flag, OID * oid)
{
  ...
  if (check_flag & CHECKDB_CHECK_PREV_LINK)
    {
      return btree_repair_prev_link (thread_p, oid, CHECK_ONLY);
    }

  if (check_flag & CHECKDB_REPAIR_PREV_LINK)
    {
      return btree_repair_prev_link (thread_p, oid, REPAIR_ALL);
    }
  ...
  valid = btree_check_by_class_oid (thread_p, oid);
  if (valid == DISK_ERROR)
    {
      return DISK_ERROR;
    }
  ...
  valid = locator_check_by_class_oid (thread_p, oid, &hfid, repair);
  if (valid == DISK_ERROR)
    {
      return DISK_ERROR;
    }
			    ...
			    OID * oids, int num_oids)
{
  ...
  if (num_oids > 0)
      ...
      for (i = 0; i < num_oids; i++)
	{
	  ...
	  isvalid = xboot_checkdb_table (thread_p, check_flag, &oids[i]);
	  if (isvalid != DISK_VALID)
	    {
	      error_code = ER_FAILED;
	    }
  ...
  if (new_lob_path == NULL)
    {
      ...
      new_lob_pathbuf2[sizeof (new_lob_pathbuf2) - 1] = '\0';
