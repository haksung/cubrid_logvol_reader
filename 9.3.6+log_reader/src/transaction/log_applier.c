...
#define CT_HA_APPLY_INFO_FOR_PLUGIN_NAME        "db_ha_apply_info_for_plugin"
#define CT_HA_PLUGIN_INFO_NAME                  "db_ha_plugin_info"
...
static char la_local_db_name[DB_MAX_IDENTIFIER_LENGTH + 1];
static char la_peer_host[MAXHOSTNAMELEN + 1];

static bool la_apply_log = true;

static bool la_enable_sql_logging = false;
static bool la_enable_plugin = false;
static bool la_enable_JVM = false;

const char *ct_class_name = CT_HA_APPLY_INFO_NAME;

LA_PLUGIN_INFO la_plugin_Info;
...
static int la_get_log_data (LOG_RECORD_HEADER * lrec, LOG_LSA * lsa,
			    ...
			    INT16 * old_type, char **old_data,
			    int *old_length, char **rec_type, char **data,
			    int *d_length);
...
static int la_get_relocation_recdes (LOG_RECORD_HEADER * lrec,
				     ...
				     char **rec_type, char **data,
				     int *d_length);
static int la_get_recdes (LOG_LSA * lsa, LOG_PAGE * pgptr,
...
static int la_create_ha_apply_info_for_plugin (void);
static int la_create_ha_plugin_info (void);
static int la_get_ha_plugin_info (void);
static LOG_LSA la_get_max_callback_lsa (void);
static int la_delete_ha_plugin_info (void);
static int la_insert_ha_plugin_info (void);
static int la_update_ha_plugin_info_first (void);
static int la_update_ha_plugin_info_status (void);
static int la_update_ha_plugin_info (void);

static int la_diff_representation_id (RECDES * old_record, RECDES * record);
...
#define LA_OUT_VALUE_COUNT      23
  ...
  if (db_find_class (ct_class_name) == NULL)
    {
      return er_errid ();
    }
  ...
  snprintf (query_buf, sizeof (query_buf), "SELECT "	/* SELECT */
	    ...
	    "   start_time "	/* 25 */
	    " FROM %s "
	    " WHERE db_name = ? and copied_log_path = ? ;", ct_class_name);

  in_value_idx = 0;
  if (la_enable_plugin)
    {
      snprintf (db_name, DB_MAX_IDENTIFIER_LENGTH, "%s@%s", la_local_db_name,
		la_plugin_Info.host_name);
      db_make_varchar (&in_value[in_value_idx++], 255, (char *) db_name,
		       strlen (db_name), LANG_SYS_CODESET,
		       LANG_SYS_COLLATION);
    }
  else
    {
  ...
  if (la_enable_plugin)
    {
      snprintf (db_name, DB_MAX_IDENTIFIER_LENGTH, "%s@%s", la_local_db_name,
		la_plugin_Info.host_name);
      db_make_varchar (&in_value[in_value_idx++], 255, db_name,
		       strlen (db_name), LANG_SYS_CODESET,
		       LANG_SYS_COLLATION);
    }
  else
...
la_get_log_data (LOG_RECORD_HEADER * lrec,
		 ...
		 void **logs, INT16 * old_type, char **old_data,
		 int *old_length, char **rec_type, char **data, int *d_length)
{
  LOG_PAGE *pg;
  PGLENGTH offset;
  int length;			/* type change PGLENGTH -> int */
  ...
  *d_length = length;
  ...
  if (old_data && error == NO_ERROR)
    {
      rec_len = DB_SIZEOF (INT16);

      if (is_undo_zip)
	{
	  *old_length = (la_Info.undo_unzip_ptr)->data_length - rec_len;
	  *old_data = (char *) malloc (*old_length);

	  memcpy (old_type, (la_Info.undo_unzip_ptr)->log_data, rec_len);
	  memcpy (*old_data, (la_Info.undo_unzip_ptr)->log_data + rec_len,
		  *old_length);
	}
      else
	{
      ...
      else
	{
	  error =
	    la_get_log_data (tmp_lrec, &lsa, pg,
			     ...
			     NULL, NULL, NULL, rec_type, data, d_length);
	}
 ...
 */
static int
la_get_recdes (LOG_LSA * lsa, LOG_PAGE * pgptr, RECDES * old_recdes,
...
la_apply_delete_log (LA_ITEM * item)
{
  ...
  if (error != NO_ERROR)
  ...
  else
    {
      /* plugin */
      if (la_enable_plugin
	  && LA_PLUGIN_STATE_IS_MASTER (la_plugin_Info.status)
	  && LA_PLUGIN_IS_DELETE (la_plugin_Info.message_type))
	{
	  if (!la_enable_JVM)
	    {
	      error = la_plugin_start (&la_plugin_Info, la_local_db_name);
	      if (error != NO_ERROR)
		{
		  er_log_debug (ARG_FILE_LINE,
				"Cannot start applylogdb plugin");
		  return ER_PLUGIN_JVM_FATAL_EXCEPTION;
		}
	      la_enable_JVM = true;
	    }

	  error =
	    la_plugin_delete (item->class_name, mclass, &item->key,
			      &item->lsa);
	  LSA_COPY (&la_plugin_Info.sent_lsa, &item->lsa);

	  if (error == NO_ERROR)
	    {
	      la_plugin_Info.delete_cnt++;
	    }
	  else if (error == ER_PLUGIN_JVM_FATAL_EXCEPTION)
	    {
	      la_plugin_Info.fatal_cnt++;
	    }
	  else if (error == ER_PLUGIN_JVM_WARNING_EXCEPTION)
	    {
	      la_plugin_Info.warning_cnt++;
	      error = NO_ERROR;
	    }
	}
    }
...
la_apply_update_log (LA_ITEM * item)
{
  ...
  if (la_enable_plugin && LA_PLUGIN_STATE_IS_MASTER (la_plugin_Info.status)
      && LA_PLUGIN_IS_UPDATE (la_plugin_Info.message_type) && !la_apply_log)
    {
      error =
	la_get_recdes (&item->target_lsa, pgptr, &old_recdes, &recdes,
		       &rcvindex, la_Info.log_data, la_Info.rec_type, &ovfyn);
    }
  else
    {
      error =
	la_get_recdes (&item->target_lsa, pgptr, NULL, &recdes, &rcvindex,
  ...
  if (la_enable_plugin && LA_PLUGIN_STATE_IS_MASTER (la_plugin_Info.status)
      ...
      if (inst_tp == NULL)
	{
      ...
      old_inst_tp = dbt_create_object_internal (class_obj);
      if (old_inst_tp == NULL)
	{
	  error = er_errid ();
	  goto error_rtn;
      ...
      if (error != NO_ERROR)
	{
      ...
      if (old_recdes.data != NULL
	  && la_diff_representation_id (&old_recdes, &recdes) == 0)
	{
	  error =
	    la_disk_to_obj (mclass, &old_recdes, old_inst_tp, &item->key);
	  if (error != NO_ERROR)
	    {
	      goto error_rtn;
	    }
  ...
  /* plugin */
  if (la_enable_plugin && LA_PLUGIN_STATE_IS_MASTER (la_plugin_Info.status)
      && LA_PLUGIN_IS_UPDATE (la_plugin_Info.message_type))
    {
      if (!la_enable_JVM)
	{
	  error = la_plugin_start (&la_plugin_Info, la_local_db_name);
	  if (error != NO_ERROR)
	    {
	      er_log_debug (ARG_FILE_LINE, "Cannot start applylogdb plugin");
	      error = ER_PLUGIN_JVM_FATAL_EXCEPTION;
	      goto error_rtn;
	    }
	  la_enable_JVM = true;
	}
	...
	{
	  error =
	    la_plugin_update_with_apply_log (object, inst_tp, &item->key,
					     &item->lsa);
	}
      else
	{
	  error =
	    la_plugin_update (old_inst_tp, inst_tp, &item->key, &item->lsa);
	}
      LSA_COPY (&la_plugin_Info.sent_lsa, &item->lsa);

      if (error == NO_ERROR)
	{
	  la_plugin_Info.update_cnt++;
	}
      else if (error == ER_PLUGIN_JVM_FATAL_EXCEPTION)
	{
	  la_plugin_Info.fatal_cnt++;
...
la_apply_update_log_server_side (LA_ITEM * item)
{
  ...
  if (la_enable_plugin && LA_PLUGIN_STATE_IS_MASTER (la_plugin_Info.status)
      ...
      do
	{
	  ...
	  if (!la_enable_JVM)
	    {
	      rc = la_plugin_start (&la_plugin_Info, la_local_db_name);
	      if (rc != NO_ERROR)
		{
		  er_log_debug (ARG_FILE_LINE,
				"Cannot start applylogdb plugin");
...
la_apply_insert_log (LA_ITEM * item)
{
  ...
  /* retrieve the target record description */
  error = la_get_recdes (&item->target_lsa, pgptr, NULL, &recdes, &rcvindex,
			 la_Info.log_data, la_Info.rec_type, &ovfyn);
  if (error == ER_NET_CANT_CONNECT_SERVER || error == ER_OBJ_NO_CONNECT)
    {
      return ER_NET_CANT_CONNECT_SERVER;
  ...
  /* plugin */
  if (la_enable_plugin && LA_PLUGIN_STATE_IS_MASTER (la_plugin_Info.status)
      && LA_PLUGIN_IS_INSERT (la_plugin_Info.message_type))
    {
      if (!la_enable_JVM)
	{
	  error = la_plugin_start (&la_plugin_Info, la_local_db_name);
      ...
      error = la_plugin_insert (inst_tp, &item->lsa);
      LSA_COPY (&la_plugin_Info.sent_lsa, &item->lsa);

      if (error == NO_ERROR)
	{
	  la_plugin_Info.insert_cnt++;
	}
      else if (error == ER_PLUGIN_JVM_FATAL_EXCEPTION)
	{
	  la_plugin_Info.fatal_cnt++;
...
la_apply_schema_log (LA_ITEM * item)
{
  ...
  if (error != NO_ERROR)
	  ...
	  && LA_PLUGIN_STATE_IS_MASTER (la_plugin_Info.status))
	{
	  ...
	  error = la_plugin_ddl (item->class_name, ddl);
	  LSA_COPY (&la_plugin_Info.sent_lsa, &item->lsa);

	  if (error == NO_ERROR)
	    {
	      la_plugin_Info.ddl_cnt++;
	    }
	  if (error == ER_PLUGIN_JVM_FATAL_EXCEPTION)
	    {
	      la_plugin_Info.fatal_cnt++;
	    }
	  else if (error == ER_PLUGIN_JVM_WARNING_EXCEPTION)
		   ...
		   int *total_rows, LOG_PAGEID final_pageid)
{
  ...
  if (la_enable_plugin)
    {
      error = la_lock_dbname (&la_Info.db_lockf_vdes, la_local_db_name,
			      la_Info.log_path);
    }
  else
    {
  ...
  LA_PLUGIN_SET_BIT (LA_DB_SERVER_STATE_MASTER, la_plugin_Info.status);
	  ...
	  && la_need_filter_out (item) == false)
	{
	  ...
	  if (error == NO_ERROR)
		  ...
		  && la_applier_need_shutdown == true)
		{
	      ...
	      /* A fatal exception occurred in apply plug-in */
	      else if (error == ER_PLUGIN_JVM_FATAL_EXCEPTION
		       || error == ER_PLUGIN_CREATE_OBJECT_FAIL)
		{
		  la_applier_need_shutdown = true;
		  goto end;
		}
	  ...
	  if (la_enable_plugin
	      && LA_PLUGIN_STATE_IS_MASTER (la_plugin_Info.status)
	      && LA_DB_SERVER_STATE_IS_MASTER (la_plugin_Info.status))
	    {
	      if (LSA_ISNULL (&la_plugin_Info.callback_lsa))
		{
		  la_plugin_get_lsa (&la_plugin_Info);
		  la_plugin_Info.last_log_record_time =
		    la_Info.log_record_time;
		  (void) la_update_ha_plugin_info ();
		  (void) la_commit_transaction ();
		}
	      else
		{
		  la_plugin_get_lsa (&la_plugin_Info);
		       ...
		       LOG_LSA * final, LOG_PAGE * pg_ptr)
{
  ...
  switch (lrec->type)
      ...
      if (LSA_GT (final, &la_Info.committed_lsa))
	{
	  ...
	  /* get a log record time for plugin */
	  if (la_enable_plugin && eot_time != 0)
	    {
	      la_plugin_Info.last_log_record_time = eot_time;
	    }
  ...
  if (error == NO_ERROR)
    {
	  ...
	  && LA_PLUGIN_STATE_IS_MASTER (la_plugin_Info.status))
	{
	  if (LA_DB_SERVER_STATE_IS_MASTER (la_plugin_Info.status) == 0)
	    {
	      LSA_COPY (&la_plugin_Info.callback_lsa, &la_Info.required_lsa);
	    }
	  (void) la_update_ha_plugin_info ();
	}
...
la_shutdown (BOOT_CLIENT_TYPE type)
{
  ...
  if (la_enable_plugin && la_enable_JVM)
    {
      la_plugin_stop ();
      la_enable_JVM = false;
    }
  ...
  if (la_Info.db_lockf_vdes != NULL_VOLDES)
	...
	{
	  error =
	    la_unlock_dbname (&la_Info.db_lockf_vdes, la_local_db_name,
			      clear_owner);
	}
      else
	{
  ...
  if (check_copied_info)
      ...
      if (error != NO_ERROR)
	{
      ...
      *copied_append_lsa = la_Info.act_log.log_hdr->append_lsa;
      ...
      la_print_log_header (database_name, la_Info.act_log.log_hdr, true);
    }
			...
			int nxarv_num, int max_arv_count_to_delete)
{
  ...
  int i;
  LOG_PAGEID new_required_lsa_pageid;

  if (la_enable_plugin && LA_PLUGIN_STATE_IS_MASTER (la_plugin_Info.status))
    {
      new_required_lsa_pageid = la_plugin_Info.callback_lsa.pageid;
    }
  else
    {
      new_required_lsa_pageid = la_Info.required_lsa.pageid;
    }

  if (LA_LOG_IS_IN_ARCHIVE (new_required_lsa_pageid))
    {
      error =
	la_find_archive_num (&required_arv_num, new_required_lsa_pageid);
      if (error != NO_ERROR)
	{
...
la_need_filter_out (LA_ITEM * item)
{
  ...
  if (filter->type == REPL_FILTER_NONE
      || (item->log_type == LOG_REPLICATION_SCHEMA
...
la_apply_log_file (const char *database_name, const char *log_path,
		   const int max_mem_size, const char *local_db_name)
{
  int error = NO_ERROR;
  struct log_header final_log_hdr;
  LA_CACHE_BUFFER *log_buf = NULL;
  ...
  if (prm_get_bool_value (PRM_ID_HA_APPLY_PLUGIN))
    {
      la_enable_plugin = true;
      la_plugin_config_read (&la_plugin_Info);
      LSA_SET_NULL (&la_plugin_Info.sent_lsa);
      LSA_SET_NULL (&la_plugin_Info.callback_lsa);
      la_plugin_Info.insert_cnt = 0;
      la_plugin_Info.update_cnt = 0;
      la_plugin_Info.delete_cnt = 0;
      la_plugin_Info.ddl_cnt = 0;
      la_plugin_Info.fatal_cnt = 0;
      la_plugin_Info.warning_cnt = 0;
      la_plugin_Info.gc_cnt = 0;
      la_plugin_Info.status = 0;
      la_apply_log = prm_get_bool_value (PRM_ID_HA_APPLY_PLUGIN_APPLY_LOG);

      GETHOSTNAME (la_plugin_Info.host_name, MAXHOSTNAMELEN);
      connected_host = db_get_host_connected ();
      if (strncmp (la_plugin_Info.host_name, connected_host, MAXHOSTNAMELEN))
	{
	  LA_PLUGIN_CLEAR_BIT (LA_PLUGIN_STATE_MASTER, la_plugin_Info.status);
	}
      else
	{
	  LA_PLUGIN_SET_BIT (LA_PLUGIN_STATE_MASTER, la_plugin_Info.status);
	}

      strncpy (la_local_db_name, local_db_name, DB_MAX_IDENTIFIER_LENGTH);
      s = strchr (la_local_db_name, '@');
      if (s)
	{
	  *s = '\0';
	}

      ct_class_name = CT_HA_APPLY_INFO_FOR_PLUGIN_NAME;
      (void) la_create_ha_apply_info_for_plugin ();
      (void) la_create_ha_plugin_info ();
      (void) la_commit_transaction ();
    }
  ...
  /* start applylogdb plug-in */
  if (la_enable_plugin)
    {
      error = la_get_ha_plugin_info ();
      if (error != NO_ERROR)
	{
	  error = la_insert_ha_plugin_info ();
	  if (error != NO_ERROR)
	    {
	      er_log_debug (ARG_FILE_LINE,
			    "Cannot insert db_ha_apply_plugin");
	      return error;
	    }
	}
      callback_lsa = la_get_max_callback_lsa ();
      LSA_COPY (&la_plugin_Info.callback_lsa, &callback_lsa);

      (void) la_update_ha_plugin_info_first ();
      (void) la_commit_transaction ();

      if (!LSA_ISNULL (&la_plugin_Info.callback_lsa))
	{
	  LSA_COPY (&la_Info.required_lsa, &la_plugin_Info.callback_lsa);
	  LSA_COPY (&la_Info.committed_lsa, &la_Info.required_lsa);
	}
    }
  ...
  do
	     ...
	     && la_applier_need_shutdown == false)
	{
	  ...
	  if (la_enable_plugin)
		...
		{
		  LA_PLUGIN_SET_BIT (LA_DB_SERVER_STATE_MASTER,
				     la_plugin_Info.status);
		}
	      else
		{
		  LA_PLUGIN_CLEAR_BIT (LA_DB_SERVER_STATE_MASTER,
				       la_plugin_Info.status);
		}

	      /* synchronize the callback lsa of the plugin slave */
	      if (LA_PLUGIN_STATE_IS_MASTER (la_plugin_Info.status) == 0)
		{
		  callback_lsa = la_get_max_callback_lsa ();
		  LSA_COPY (&la_plugin_Info.callback_lsa, &callback_lsa);
		}

	      (void) la_update_ha_plugin_info_status ();
	      (void) la_commit_transaction ();
	    }
	  ...
	  /* wait plugin of slave side */
	  if (la_enable_plugin
	      && (LA_PLUGIN_STATE_IS_MASTER (la_plugin_Info.status) == 0)
	      && la_Info.final_lsa.pageid >=
	      la_plugin_Info.callback_lsa.pageid)
	    {
	      mode = boot_change_ha_mode (HA_SERVER_STATE_NA, false, 0);
	      if (mode != HA_SERVER_STATE_ACTIVE
		  && mode != HA_SERVER_STATE_TO_BE_ACTIVE)
		{
		  la_applier_need_shutdown = true;
		  break;
		}
...
/*
 * la_create_ha_apply_info_for_plugin() - create db_ha_apply_info_for_plugin table
 *   return: NO_ERROR or error code
 *
 * Note:
 *     called by APPLY thread
...
static int
la_create_ha_apply_info_for_plugin (void)
{
  int error = NO_ERROR;
  char query_buf[LA_QUERY_BUF_SIZE];
  ...
  snprintf (query_buf, sizeof (query_buf),
	    ...
	    "copied_log_path CHARACTER VARYING(4096) NOT NULL, "
	    "committed_lsa_pageid BIGINT NOT NULL, "
	    "committed_lsa_offset INTEGER NOT NULL, "
	    ...
	    "fail_counter BIGINT, "
	    "start_time DATETIME, "
	    "CONSTRAINT pk PRIMARY KEY(db_name, copied_log_path) "
	    "); ", CT_HA_APPLY_INFO_FOR_PLUGIN_NAME);

  error = la_update_query_execute (query_buf, true);

  return error;
}

/*
 * la_create_ha_plugin_info() - create db_ha_plugin_info table
 *   return: NO_ERROR or error code
 *
 * Note:
 *     called by APPLY thread
...
la_create_ha_plugin_info (void)
{
  ...
  snprintf (query_buf, sizeof (query_buf),
	    ...
	    "sent_lsa_pageid BIGINT NOT NULL, "
	    "sent_lsa_offset INTEGER NOT NULL, "
	    "callback_lsa_pageid BIGINT NOT NULL, "
	    "callback_lsa_offset INTEGER NOT NULL, "
	    "start_time DATETIME, "
	    "last_action_time DATETIME, "
	    "log_record_time DATETIME, "
	    "insert_success_count BIGINT, "
	    "update_success_count BIGINT, "
	    "delete_success_count BIGINT, "
	    "ddl_success_count BIGINT, "
	    "fatal_exception_count BIGINT, "
	    "warning_exception_count BIGINT, "
	    "gc_cnt INTEGER, "
	    "[status] INTEGER, "
	    "CONSTRAINT pk PRIMARY KEY(db_name, copied_log_path, host_name) "
	    "); ", CT_HA_PLUGIN_INFO_NAME);
 ...
 */
static int
la_get_ha_plugin_info (void)
{
#define LA_IN_VALUE_COUNT       3
#define LA_OUT_VALUE_COUNT      11
  ...
  if (db_find_class (CT_HA_PLUGIN_INFO_NAME) == NULL)
    {
      return er_errid ();
    }
  ...
  snprintf (query_buf, sizeof (query_buf), "SELECT "	/* SELECT */
	    "   sent_lsa_pageid, "	/* 1 */
	    "   sent_lsa_offset, "	/* 2 */
	    "   callback_lsa_pageid, "	/* 3 */
	    "   callback_lsa_offset, "	/* 4 */
	    "   insert_success_count, "	/* 5 */
	    "   update_success_count, "	/* 6 */
	    "   delete_success_count, "	/* 7 */
	    "   ddl_success_count, "	/* 8 */
	    "   fatal_exception_count, "	/* 9 */
	    "   warning_exception_count, "	/* 10 */
	    "   gc_cnt "	/* 11 */
	    " FROM %s WHERE db_name = ? and "	/* 1 */
	    "   copied_log_path = ? and "	/* 2 */
	    "   host_name = ?; ", CT_HA_PLUGIN_INFO_NAME);	/* 3 */
  ...
  /* 3. host_name */
  db_make_varchar (&in_value[in_value_idx++], 255, la_plugin_Info.host_name,
		   strlen (la_plugin_Info.host_name),
		   LANG_SYS_CODESET, LANG_SYS_COLLATION);
  ...
  if (error > 0)
	...
	case DB_CURSOR_SUCCESS:
	  ...
	  /* 1 ~ 2. sent_lsa */
	  la_plugin_Info.sent_lsa.pageid =
	    DB_GET_BIGINT (&out_value[out_value_idx++]);
	  la_plugin_Info.sent_lsa.offset =
	    DB_GET_INTEGER (&out_value[out_value_idx++]);

	  /* 3 ~ 4. callback_lsa */
	  la_plugin_Info.callback_lsa.pageid =
	    DB_GET_BIGINT (&out_value[out_value_idx++]);
	  la_plugin_Info.callback_lsa.offset =
	    DB_GET_INTEGER (&out_value[out_value_idx++]);
...
/*
 * la_get_max_callback_lsa() - 
 *   return: NO_ERROR or error code
 *
 * Note:
 */
static LOG_LSA
la_get_max_callback_lsa (void)
{
#define LA_IN_VALUE_COUNT       2
#define LA_OUT_VALUE_COUNT      2
  ...
  snprintf (query_buf, sizeof (query_buf), "SELECT "	/* SELECT */
	    "   callback_lsa_pageid, "	/* 1 */
	    "   callback_lsa_offset "	/* 2 */
	    " FROM %s WHERE db_name = ? and "	/* 1 */
	    "       copied_log_path = ? "	/* 2 */
	    " ORDER BY callback_lsa_pageid DESC, "
	    "          callback_lsa_offset DESC LIMIT 1; ",
	    CT_HA_PLUGIN_INFO_NAME);
  ...
  if (error > 0)
	...
	case DB_CURSOR_SUCCESS:
	  ...
	  /* 1 ~ 2. callback_lsa */
	  callback_lsa.pageid = DB_GET_BIGINT (&out_value[out_value_idx++]);
	  callback_lsa.offset = DB_GET_INTEGER (&out_value[out_value_idx++]);
...
/*
 * la_delete_ha_plugin_info() - delete db_ha_plugin_info table
 *   return: NO_ERROR or error code
 *
 * Note:
 *     called by APPLY thread
...
/*
 * la_insert_ha_plugin_info() - insert db_ha_plugin_info table
 *   return: NO_ERROR or error code
 *
 * Note:
 *     called by APPLY thread
...
static int
la_insert_ha_plugin_info (void)
{
#define LA_IN_VALUE_COUNT       7
  ...
  snprintf (query_buf, sizeof (query_buf), "INSERT INTO %s ("	/* INSERT */
	    ...
	    "   sent_lsa_pageid, "	/* 4 */
	    "   sent_lsa_offset, "	/* 5 */
	    "   callback_lsa_pageid, "	/* 6 */
	    "   callback_lsa_offset,  "	/* 7 */
	    "   insert_success_count, update_success_count, delete_success_count, ddl_success_count, fatal_exception_count, warning_exception_count, gc_cnt ) "	/* counting */
	    " VALUES ( ?, "	/*  1. db_name */
	    "   ?, "		/*  2. copied_log_path */
	    "   ?, "		/*  3. host_name */
	    "   ?, "		/*  4. sent_lsa_pageid */
	    "   ?, "		/*  5. sent_lsa_offset */
	    "   ?, "		/*  6. callback_lsa_pageid */
	    "   ?,  "		/*  7. callback_lsa_offset */
	    "   0, 0, 0, 0, 0, 0, 0 ) ;", CT_HA_PLUGIN_INFO_NAME);
  ...
  /* 3. host_name */
  db_make_varchar (&in_value[in_value_idx++], 4096, la_plugin_Info.host_name,
		   strlen (la_plugin_Info.host_name),
		   LANG_SYS_CODESET, LANG_SYS_COLLATION);

  /* 4 ~ 5. sent_lsa */
  db_make_bigint (&in_value[in_value_idx++], la_plugin_Info.sent_lsa.pageid);
  db_make_int (&in_value[in_value_idx++], la_plugin_Info.sent_lsa.offset);

  /* 6 ~ 7. callback_lsa */
  db_make_bigint (&in_value[in_value_idx++],
		  la_plugin_Info.callback_lsa.pageid);
  db_make_int (&in_value[in_value_idx++], la_plugin_Info.callback_lsa.offset);
...
/*
 * la_update_ha_plugin_info_first() - update db_ha_plugin_info table
 *   return: NO_ERROR or error code
 *
 * Note:
 *     called by APPLY thread
 */
static int
la_update_ha_plugin_info_first (void)
{
#define LA_IN_VALUE_COUNT       4
  int error = NO_ERROR;
  int i;
  int in_value_idx;
  DB_DATETIME datetime;
  ...
  snprintf (query_buf, sizeof (query_buf), "UPDATE %s SET "	/* UPDATE */
	    "   start_time = SYS_DATETIME, last_action_time = SYS_DATETIME, [status] = ? "	/* 1 */
	    " WHERE db_name = ? and "	/* 2 */
	    "      copied_log_path = ? and "	/* 3 */
	    "      host_name = ?; ", CT_HA_PLUGIN_INFO_NAME);	/* 4 */

  in_value_idx = 0;

  /* 1. status */
  db_make_int (&in_value[in_value_idx++], la_plugin_Info.status);
...
/*
 * la_update_ha_plugin_info_status() - update db_ha_plugin_info table
 *   return: NO_ERROR or error code
 *
 * Note:
 *     called by APPLY thread
 */
static int
la_update_ha_plugin_info_status (void)
{
#define LA_IN_VALUE_COUNT       5
  ...
  snprintf (query_buf, sizeof (query_buf), "UPDATE %s SET "	/* UPDATE */
	    "   last_action_time = SYS_DATETIME, log_record_time = ?, "	/* 1 */
	    "   [status] = ? "	/* 2 */
	    " WHERE db_name = ? and "	/* 3 */
	    "      copied_log_path = ? and "	/* 4 */
	    "      host_name = ?; ", CT_HA_PLUGIN_INFO_NAME);	/* 5 */

  in_value_idx = 0;

  /* 1. log_record_time */
  db_localdatetime (&la_plugin_Info.last_log_record_time, &datetime);
  db_make_datetime (&in_value[in_value_idx++], &datetime);
...
/*
 * la_update_ha_plugin_info() - update db_ha_plugin_info table
 *   return: NO_ERROR or error code
 *
 * Note:
 *     called by APPLY thread
 */
static int
la_update_ha_plugin_info (void)
{
#define LA_IN_VALUE_COUNT       16
  int error = NO_ERROR;
  int i;
  int in_value_idx;
  DB_DATETIME datetime;
  ...
  snprintf (query_buf, sizeof (query_buf), "UPDATE %s SET "	/* UPDATE */
	    ...
	    "   callback_lsa_pageid = ?, "	/* 3 */
	    "   callback_lsa_offset = ?, "	/* 4 */
	    "   last_action_time = SYS_DATETIME, log_record_time = ?, "	/* 5 */
	    "   insert_success_count = ?, "	/* 6 */
	    "   update_success_count = ?, "	/* 7 */
	    "   delete_success_count = ?, "	/* 8 */
	    "   ddl_success_count = ?, "	/* 9 */
	    "   fatal_exception_count = ?, "	/* 10 */
	    "   warning_exception_count = ?, "	/* 11 */
	    "   gc_cnt = ?, "	/* 12 */
	    "   [status] = ? "	/* 13 */
	    " WHERE db_name = ? and "	/* 14 */
	    "      copied_log_path = ? and "	/* 15 */
	    "      host_name = ?; ", CT_HA_PLUGIN_INFO_NAME);	/* 16 */
  ...
  /* 6 ~ 9. success_count */
  db_make_bigint (&in_value[in_value_idx++], la_plugin_Info.insert_cnt);
  db_make_bigint (&in_value[in_value_idx++], la_plugin_Info.update_cnt);
  db_make_bigint (&in_value[in_value_idx++], la_plugin_Info.delete_cnt);
  db_make_bigint (&in_value[in_value_idx++], la_plugin_Info.ddl_cnt);

  /* 10 ~ 11. exception_count */
  db_make_bigint (&in_value[in_value_idx++], la_plugin_Info.fatal_cnt);
  db_make_bigint (&in_value[in_value_idx++], la_plugin_Info.warning_cnt);

  /* 12. gc_cnt */
  db_make_int (&in_value[in_value_idx++], la_plugin_Info.gc_cnt);
...
/*
 * la_diff_representation_id() - compare two representation IDs
 *   return: NO_ERROR or error code
 *
 * Note:
 *     called by APPLY thread
...
static int
la_diff_representation_id (RECDES * old_record, RECDES * record)
{
  OR_BUF orep, *orepp;
  unsigned int old_repid_bits, repid_bits;
  int old_repid, repid;
  int rc = NO_ERROR;
  orepp = &orep;

  or_init (orepp, old_record->data, old_record->length);
  old_repid_bits = or_get_int (orepp, &rc);
  old_repid = old_repid_bits & ~OR_BOUND_BIT_FLAG & ~OR_OFFSET_SIZE_FLAG;

  or_init (orepp, record->data, record->length);
  repid_bits = or_get_int (orepp, &rc);
  repid = repid_bits & ~OR_BOUND_BIT_FLAG & ~OR_OFFSET_SIZE_FLAG;

  return (old_repid - repid);
}
